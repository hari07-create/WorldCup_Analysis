...
1. **Data Sourcing:** Historical tournament records sourced from Kaggle (`world_cup_results.xlsx`).
2. **Data Wrangling & Cleaning (Python):** Resolving structural format changes, correcting corrupted numerical strings, handling encoding artifacts, and parsing unstructured textual event logs.
3. **Exploratory Data Analysis (EDA):** Extracting statistical trends, identifying powerhouses, and mapping out goal distributions.
4. **Data Visualization (Tableau & Python):** Building clean, high-resolution visual components for quick insights alongside a dynamic user dashboard.
...
├── 2_top_scoring_countries.png      # Plot 2: Top scoring countries bar chart
